...
class ChatServer:
    """Класс реализует функционал сокетного сервера посредством классов StreamReader
    и StreamWriter, отслеживает подключение клиентов к чату и оповещает всех подключенных
    клиентов о подключении/отключении клиентов"""
    def __init__(self):
...
async def main():
    ...
    await chat_server.start_chat_server('127.0.0.1', 8080)
